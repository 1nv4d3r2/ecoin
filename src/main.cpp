...
#if defined(NDEBUG)
# error "Ecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbe8bd4e58bb614d4460d72492a5acdfc6002b2375090ac75bd040f69edab48e5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ecoin: starting difficulty is 1 / 2^12
...
{
    // Ecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 940000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 940000); // Ecoin: 940k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ecoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Ecoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe8945234eec4445dfe9a1336ab9c6cdd52bc92431f346fd87dade392a3c3d0f2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BR Brasil 21/Jan/2020 Criada E-coin, Cripto moeda anonima do Brasil,";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0463e2f3c33f6352973c8231602764c66be9e37a4b650877cc25f2f829417d3e1dc5e793f9b4fa31c72379a70f43b39d9fda9e4eebc456816073154fe84f693ccb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1579624214;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089003613;

        if (fTestNet)
        {
            block.nTime    = 1579624184;
            block.nNonce   = 388905125;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6e12f1153bd6147eb9ecdd55cddedd8294250f85d156365bafe6d9390717e0cb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xca, 0xbf, 0xda }; // Ecoin: increase each by adding 2 to bitcoin's value.
...
void static EcoinMiner(CWallet *pwallet)
{
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EcoinMiner, pwallet));
}
